...
**Goal:** Build `scripts/refresh-docs.sh` — an explicitly-triggered pipeline that audits CLAUDE.md against the live codebase, appends AI-analyzed issues/next-steps to HANDOFF.md, extracts new skills from git history, verifies the post-commit hook, and commits all changes after a diff + confirmation prompt.
...
═══ Step 5: Post-commit hook verification ═══
  [refresh] Hook PASS — post-commit hook exits 0.
...
- [ ] **Step 5: Verify the commit and that HANDOFF.md was regenerated by the hook**
