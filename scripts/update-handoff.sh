...
# Regenerates the local-only HANDOFF.md artifact with current project state.
# Run manually when preparing a detailed local checkpoint, handoff, or release note refresh.
#
...
>
> Auto-generated by \`scripts/update-handoff.sh\` when a local checkpoint or handoff is requested.
