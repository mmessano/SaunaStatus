...
# Called automatically by a pre-commit hook wrapper, or run manually.
#
# Environment variables:
#   SKIP_BUILD=1   Skip PlatformIO compilation (default: auto — skips if no
#                  relevant C/C++ files changed)
#   FORCE_BUILD=1  Always run PlatformIO build regardless of changed files
#   HANDOFF_USE_STAGED=1  Inspect staged changes instead of the last commit.
#
# Usage:
...
PRE_COMMIT_MODE="${HANDOFF_USE_STAGED:-0}"
HEADER_NOTE=""
...
if [[ "${FORCE_BUILD:-0}" == "1" ]]; then
    ...
    if [[ "$PRE_COMMIT_MODE" == "1" ]]; then
        CHANGED_CPP="$(git diff --cached --name-only \
...
if [[ "$PRE_COMMIT_MODE" == "1" ]]; then
    LAST_COMMIT="${LAST_COMMIT} (HEAD before pending commit)"
    HEADER_NOTE="> Generated from staged changes before commit; the latest committed revision above is the current HEAD prior to the pending commit."
fi
...
if [[ "$RUN_BUILD" == "1" ]]; then
    ...
    elif [[ "$PRE_COMMIT_MODE" == "1" ]]; then
        BUILD_NOTE="No staged C/C++ source files changed — build skipped. Run FORCE_BUILD=1 bash scripts/update-handoff.sh to override."
    else
...
>
> Auto-generated by \`scripts/update-handoff.sh\` before commit via the pre-commit hook, or run manually.
> Do not edit by hand — it will be overwritten on the next hook run or manual regeneration.
